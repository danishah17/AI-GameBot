...
class Bot:
    ...
    def log_to_csv(self, round_id, state, action_idx, csv_writer):
        timestamp = time.time()
        row = [
            round_id,
            timestamp,
    ...
    def generate_dataset(self, num_rounds=100, csv_file="game_dataset.csv"):
        # Initialize CSV
        with open(csv_file, 'w', newline='') as f:
            ...
            for round_id in range(num_rounds):
                ...
                step = 0
                while True:
                    # Mock GameState (simplified opponent)
                    game_state = self.mock_game_state(step, round_id)
                    
                    # Run fight
                    command = self.fight(game_state, "1", log_to_csv=True, csv_writer=writer)
                    ...
                    step += 1
                    # Simulate opponent action (random)
                    self.simulate_opponent(game_state)
                    
                    # Update mock state (simplified)
                    if step % 10 == 0:
                        game_state.player1.health -= random.randint(0, 5)
                        game_state.player2.health -= random.randint(0, 5)
                        game_state.timer -= 1
                        if game_state.player1.health <= 0 or game_state.player2.health <= 0 or game_state.timer <= 0:
                            game_state.is_round_over = True
                            
                print(f"Completed round {round_id + 1}/{num_rounds}")

    def mock_game_state(self, step, round_id):
        # Mock Player objects
        player1_buttons = Buttons()
        player2_buttons = Buttons()
        player1 = Player({
            'x_coord': 100 + step % 50,
            'health': 100,
            'player_buttons': player1_buttons
        })
        player2 = Player({
            'x_coord': 200 - step % 50,
            'health': 100,
            'player_buttons': player2_buttons
        })
        
        # Mock GameState
        input_dict = {
            'p1': {
                'x_coord': player1.x_coord,
                'health': player1.health,
                'player_buttons': player1_buttons
            },
            'p2': {
                'x_coord': player2.x_coord,
                'health': player2.health,
                'player_buttons': player2_buttons
            },
            'timer': 60 - step // 10,
            'result': '',
            'round_started': step > 0,
            'round_over': False
        }
        return GameState(input_dict)

    def simulate_opponent(self, game_state):
        # Random opponent actions
        opponent_buttons = game_state.player2.player_buttons
        opponent_buttons.Y = random.choice([True, False])
        opponent_buttons.B = random.choice([True, False])
        opponent_buttons.left = random.choice([True, False])
        opponent_buttons.right = random.choice([True, False])

    def fight(self, current_game_state, player, log_to_csv=False, csv_writer=None):
        ...
        if self.prev_health[player] is None:
            self.prev_health[player] = current_game_state.player1.health if player == "1" else current_game_state.player2.health
            self.prev_health['2' if player == "1" else '1'] = current_game_state.player2.health if player == "1" else current_game_state.player1.health
        ...
        if self.exe_code != 0:
            ...
            if log_to_csv and csv_writer:
                self.log_to_csv(current_game_state.timer, state, action_idx, csv_writer)
        ...
        if current_health < self.prev_health[player]:
            reward -= 10
        if opponent_health < prev_opponent_health:
            reward += 10
        if current_game_state.is_round_over:
